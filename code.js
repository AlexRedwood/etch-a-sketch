...
for (i = 0; i < 256; i++) {
  ...
  div.setAttribute(
    "style",
    "background-color: #c1dacb;; border:1px solid #505050"
  );
...
function uBoard() {
  ...
  let userChoice = window.prompt(
    "Enter desireable board size please: (from 1 to 100) "
  );
  //make more rows and columns
  if (userChoice === null) return;
  if (userChoice <= 0 || userChoice > 100)
    return alert("Pick a number from 1 to 100 please."), uBoard();

  if (isNaN(userChoice))
    return (
      alert(
        "Board size is measured in numbers, sweetheart. Please, type one next time."
      ),
      uBoard()
    );
  ...
  board.style =
    ...
    Math.floor(userChoice) +
    ", minmax(1px, 1000px)); grid-template-rows: repeat(" +
    Math.floor(userChoice) +
    ", minmax(1px, 1000px));";
